...
data O = O deriving (Generic, Singleton)
...
data True' = True' deriving (Generic, Singleton)
...
instance CMONO f p => Singleton (Sub f p) where
    singleton = Sub singleton singleton
...
instance COmega f p g q => Singleton (Omega f p g q) where
    singleton = Omega singleton singleton singleton singleton
